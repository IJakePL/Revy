...
module.exports.validateGuild = async (req, res, next) => {
  res.locals.guild = await res.locals.guilds.find(
    (g) => g.id === req.params.id
  );
  return (await res.locals.guild) ? next() : res.render("errors/404");
};

module.exports.validateUser = async (req, res, next) => {
  return (await res.locals.user) ? next() : res.render("errors/401");
};
